...
<script type="text/javascript">
    ...
    function searchNotice() {//显示查询信息
        ...
        if (keywords == "") {
            ...
            window.location.href = "${pageContext.request.contextPath}/NoticeServlet?method=pageNotice&u_page=" + u_page + "&keywords=" + keywords;
        }
    }
...
<div class="right-down-middle">
    <div class="right-down-up">
        <div class="right-down-up-left">
            <a href="${pageContext.request.contextPath}/addNotice.jsp">
                <button type="button" class="btn btn-default">发布通知</button>
            </a>
        </div>
        <div class="right-down-up-right">
            <input id="userSearch" style="height: 35px;" type="text" value="${keywords}" placeholder="通知标题">
            <button type="button" class="btn btn-primary btn-sm" id="search" onclick="searchNotice()">查询</button>
            <a href="${pageContext.request.contextPath}/NoticeServlet?method=mainNotice">
    ...
    <table class="table table-striped">
        ...
        <c:forEach items="${noticeList}" var="notice">
            <tr>
                ...
                <td>
                    <a href="javascript:opennotice('${notice.title}','${notice.publishTime}',
            '${notice.beganTime}','${notice.endTime}','${notice.content}')">查看</a>
                </td>
                <td>
                    <a class="remove" href="javascript:removeNotice('${notice.id}','${notice.title}')">删除</a>
                </td>
                <td>${notice.beganTime}</td>
...
<div id="myModal-notice" class="modal-notice">
    ...
    <div class="modal-content-notice">
        ...
        <div class="modal-body-notice">
            ...
            <span rows="20" cols="60" id="content"></span>
            <%--            </div>--%>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function removeNotice(id, title) {
        ...
        if (r == true) {
            $.post({
                url: "${pageContext.request.contextPath}/NoticeServlet?method=removeNotice",
                data: {"id": id},
                dataType: "text",
